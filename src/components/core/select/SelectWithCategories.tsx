...
const SelectWithCategories: React.FC<SelectWithCategoriesProps> = ({
    ...
    return (
        <div ref={selectRef} className="relative">
            <div
                className={`h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 pr-11 text-sm shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 cursor-pointer ${selectedValue
            ...
            >
                ...
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                    <i className={`fa-duotone fa-solid fa-chevron-${isOpen ? 'up' : 'down'} text-gray-400`}></i>
                </div>
            </div>
                    ...
                    {options.map((category, index) => (
                        <div key={index}>
                            ...
                            {category.options.map((option) => (
                                <div
                                    key={option.value}
                                    className={`relative cursor-pointer select-none py-2 pl-4 pr-9 hover:bg-gray-100 dark:hover:bg-gray-800 ${selectedValue === option.value
                                        ? 'bg-gray-100 dark:bg-gray-800'
                                        : ''
                                        }`}
                                    onClick={() => handleSelect(option.value)}
                                >
                                    <span className={`block truncate ${selectedValue === option.value
