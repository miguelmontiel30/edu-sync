...
// Types
import { Document, DocumentType, Message, MessageType } from './module-utils/types';
...
export default function StudentProfilePage() {
    ...
    const { tabs, changeTab, academicTabs, academicTab, changeAcademicTab } =
    ...
    if (!student && !loadingState.student) {
        return (
            <div className="flex h-screen flex-col items-center justify-center">
                <IconFA icon="exclamation-circle" size="xl" className="mb-4 text-error-500" />
                <h2 className="text-xl font-medium">Estudiante no encontrado</h2>
                <p className="mt-2 text-gray-500">
                    No se pudo encontrar el estudiante con ID: {studentId}
                </p>
                <Button variant="primary" className="mt-4" onClick={handleBackToStudents}>
    ...
    return (
        <div className="mx-auto max-w-screen-2xl md:p-6">
            ...
            <PageBreadcrumb
                ...
                items={[{ title: 'Estudiantes', path: '/admin-dashboard/admin-students' }]}
            />
                        ...
                        <TabsTrigger key={tab.id} value={tab.id} className="px-4 py-2">
                            ...
                            {tab.id === 'documents' &&
                                mockData.documents.filter(
                                    (doc: Document) => doc.status === 'pendiente',
                                ).length > 0 && (
                                    <NotificationBadge
                                        count={
                                            mockData.documents.filter(
