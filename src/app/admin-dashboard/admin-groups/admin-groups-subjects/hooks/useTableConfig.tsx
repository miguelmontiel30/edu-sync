...
export const useTableConfig = ({
    ...
    const subjectColumns: Column<GroupSubjectAssignment>[] = useMemo(() => [
        {
            ...
            render: (item: GroupSubjectAssignment) => (
                <div className="flex items-center justify-center">
                    ...
                    <span className="text-sm font-outfit font-bold">{item.subject?.name || 'Materia sin nombre'}</span>
                </div>
            )
        },
        {
            ...
            header: 'Descripción',
            render: (item: GroupSubjectAssignment) => <span className="text-sm font-outfit">{item.subject?.description || 'Sin descripción'}</span>
        },
        {
            key: 'teacher',
        ...
        },
        {
            ...
            render: (item: GroupSubjectAssignment) => (
                <div className="flex items-center">
                    ...
                    <span className="text-sm font-outfit font-bold">{item.student_count || 0}</span>
                </div>
            )
        }
    ], []);
    ...
    const subjectActionButtons: ActionButton[] = useMemo(() => [
        {
            ...
            showCondition: (item: GroupSubjectAssignment) => item.group_subject_id !== undefined
        },
        {
    ...
    const deleteConfirmModalConfig = useMemo(() => ({
        ...
        description: 'Esta acción eliminará la materia del grupo actual, pero no afectará a la materia en otros grupos.',
    ...
    const deletedSubjectsListConfig: DeletedItemsListConfig<DeletedGroupSubject> = useMemo(() => ({
        ...
        searchNoResultsMessage: 'No se encontraron materias eliminadas que coincidan con la búsqueda',
        ...
        columns: [
            {
                ...
                render: (item: DeletedGroupSubject) => (
                    <div className="flex items-center justify-center">
                        <IconFA icon="book" className="mr-2 text-indigo-600" />
                        <span className="text-sm font-outfit font-medium">{item.subject?.name || 'Materia sin nombre'}</span>
                    </div>
                )
            },
            {
            ...
            },
            {
                ...
                render: (item: DeletedGroupSubject) => {
                    ...
                    return <Badge variant="light" color="warning">Sin profesor</Badge>;
                }
            },
        ]
    }), [handleRestoreSubject]);
