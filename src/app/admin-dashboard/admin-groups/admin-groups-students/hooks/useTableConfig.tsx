...
import {
    formatDate,
    getGenderIconColor,
} from '@/app/admin-dashboard/admin-students/module-utils/utils';
...
export function useTableConfig({
    ...
    const studentColumns: Column<Student>[] = useMemo(
        () => [
                    ...
                    <div className="flex items-center">
                        {student.image_url ? (
                            ...
                            <div className="mr-2 flex h-8 w-8 items-center justify-center rounded-full bg-gray-200 dark:bg-gray-700">
                                <IconFA
                                    icon="user"
                                    className={getGenderIconColor(student.gender?.code)}
                                />
                            </div>
                        )}
            ...
            },
            {
                ...
                sortable: true,
                render: (student: Student) => (
                    <span className="font-mono text-xs uppercase">{student.curp}</span>
                ),
            },
            {
                key: 'age',
                header: 'Edad',
                sortable: true,
                render: (student: Student) => <span>{student.age} años</span>,
            },
            {
                key: 'birth_date',
                header: 'Fecha de Nacimiento',
                sortable: true,
                render: (student: Student) => <span>{formatDate(student.birth_date)}</span>,
            },
            {
                key: 'status',
                header: 'Estado',
    ...
    const inactiveStudentListConfig: DeletedItemsListConfig<Student> = useMemo(
        () => ({
            ...
            searchNoResultsMessage:
                'No se encontraron estudiantes inactivos que coincidan con la búsqueda',
            itemsPerPage: 5,
            maxHeight: '400px',
    ...
    const deletedStudentListConfig: DeletedItemsListConfig<Student> = useMemo(
        () => ({
            ...
            description:
                'Estudiantes que han sido dados de baja del grupo pero pueden ser restaurados',
            ...
            searchNoResultsMessage:
                'No se encontraron estudiantes eliminados que coincidan con la búsqueda',
            itemsPerPage: 5,
            maxHeight: '400px',
            columns: studentColumns,
    ...
    const deleteConfirmModalConfig = useMemo(
        () => ({
            ...
            message:
                'Estás a punto de eliminar a un estudiante del grupo. Esta acción no eliminará permanentemente al estudiante del sistema, sino que lo marcará como inactivo en este grupo.',
            confirmButton: 'Sí, Eliminar',
            cancelButton: 'Cancelar',
            activeWarning:
                'Este estudiante está activo en el grupo. Al eliminarlo, se perderá su asignación actual.',
        }),
        [],
    );
