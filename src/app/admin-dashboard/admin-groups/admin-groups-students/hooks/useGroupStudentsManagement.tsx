...
} from '@/app/admin-dashboard/admin-groups/admin-groups-students/module-utils/types';
...
export const useGroupStudentsManagement = () => {
    ...
    const loadGroups = useCallback(async () => {
        ...
        try {
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                groups: 'No se pudieron cargar los grupos. Intenta nuevamente.',
            }));
        } finally {
    ...
    const loadAllStudents = useCallback(async () => {
        ...
        try {
            ...
            const activeStudentsData = await fetchActiveStudentsByGroup(
                selectedGroup.id,
            ...
            const inactiveStudentsData = await fetchInactiveStudentsByGroup(
                selectedGroup.id,
                session.school_id as number,
            );
            ...
            const deletedStudentsData = await fetchDeletedStudentsByGroup(
                selectedGroup.id,
                session.school_id as number,
            );
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                students: 'No se pudieron cargar los estudiantes del grupo.',
            }));
            setActiveStudents([]);
            setInactiveStudents([]);
            setDeletedStudents([]);
    ...
    const loadAvailableStudents = useCallback(async () => {
        ...
        try {
            ...
            const allStudentsRaw = await getAvailableStudentsForNewGroupAssignment(
                session.school_id as number,
            );
            ...
            const studentsAvailable = filterAvailableStudents(
                allStudentsFormatted,
                allCurrentStudents,
            );
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                students: 'No se pudieron cargar los estudiantes disponibles.',
            }));
    ...
    const handleAddStudents = async (studentIds: number[]) => {
        ...
        try {
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                saving: 'No se pudieron asignar los estudiantes.',
            }));
        } finally {
    ...
    const handleDeleteStudent = async (studentGroupId: number) => {
        ...
        try {
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                saving: 'No se pudo eliminar el estudiante.',
            }));
        } finally {
            setLoadingState((prev: LoadingStates) => ({ ...prev, saving: false }));
        }
    ...
    const handleRestoreStudent = async (studentGroupId: number) => {
        ...
        try {
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                saving: 'No se pudo restaurar el estudiante.',
            }));
        } finally {
            setLoadingState((prev: LoadingStates) => ({ ...prev, saving: false }));
        }
    ...
    const handleUpdateStudentStatus = async (studentGroupId: number, newStatusId: number) => {
        ...
        try {
            ...
            if (!success) {
                throw new Error(
                    'Error en la respuesta del servidor al actualizar el estado del estudiante',
                );
            }
            ...
            setErrorState((prev: ErrorStates) => ({
                ...prev,
                saving: 'No se pudo actualizar el estado del estudiante.',
            }));
        } finally {
            setLoadingState((prev: LoadingStates) => ({ ...prev, saving: false }));
        }
