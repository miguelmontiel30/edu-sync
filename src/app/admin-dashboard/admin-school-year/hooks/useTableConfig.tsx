...
export function useTableConfig({ handleEdit, handleDelete }: TableConfigProps): TableConfigs {
        ...
        () => [
            {
                ...
                render: (cycle: SchoolCycle) => (
                    <span className={textStyles.title}>{cycle.name}</span>
                ),
            },
            {
                key: 'startDate',
                ...
                sortable: true,
                render: (cycle: SchoolCycle) => (
                    <span className={textStyles.normal}>{formatDate(cycle.startDate)}</span>
                ),
            },
            {
                key: 'endDate',
                ...
                sortable: true,
                render: (cycle: SchoolCycle) => (
                    <span className={textStyles.normal}>{formatDate(cycle.endDate)}</span>
                ),
            },
            {
                key: 'groupsCount',
            ...
            },
            {
                ...
                sortable: true,
                render: (cycle: SchoolCycle) => (
                    <span className={textStyles.normal}>{cycle.averageGrade.toFixed(2)}</span>
                ),
            },
            {
                key: 'status',
                ...
                render: (cycle: SchoolCycle) => (
                    <Badge size="sm" color={getStatusColor(cycle.status)}>
                        <span className="font-outfit">{cycle.statusName}</span>
                    </Badge>
                ),
            },
        ],
    ...
    const cycleListConfig: ItemsListConfig<SchoolCycle> = useMemo(
        () => ({
            ...
            description:
                'Aquí podrás ver todos los ciclos escolares registrados, su información y gestionarlos. Puedes crear nuevos ciclos, editar los existentes o eliminarlos según sea necesario.',
    ...
    const deletedCycleListConfig: DeletedItemsListConfig<SchoolCycle> = useMemo(
        () => ({
            ...
            searchNoResultsMessage:
                'No se encontraron ciclos escolares que coincidan con la búsqueda',
            ...
            columns: [
                ...
                },
                {
                    ...
                    header: 'Grupos',
                    render: (cycle: SchoolCycle) => (
                        <span className={textStyles.normal}>{cycle.groupsCount}</span>
                    ),
                },
                {
                    key: 'studentsCount',
                    header: 'Alumnos',
                    render: (cycle: SchoolCycle) => (
                        <span className={textStyles.normal}>{cycle.studentsCount}</span>
                    ),
                },
                {
                    key: 'averageGrade',
                ...
                },
                {
                    ...
                    render: (cycle: SchoolCycle) => (
                        <Badge size="sm" color={getStatusColor(cycle.status)}>
                            <span className={textStyles.normal}>{cycle.statusName}</span>
                        </Badge>
                    ),
                },
            ],
    ...
    const deleteConfirmModalConfig = useMemo(
        () => ({
            ...
            warningMessage:
                'Estás a punto de eliminar un ciclo escolar ACTIVO. Esta acción podría afectar negativamente al funcionamiento del sistema. Por favor, asegúrate de completar el ciclo escolar o activar otro ciclo antes de continuar.',
            errorTitle: '¡Error!',
            errorMessage:
                'Ocurrió un error al intentar eliminar el ciclo escolar. Por favor, intenta nuevamente.',
        }),
        [],
    );
