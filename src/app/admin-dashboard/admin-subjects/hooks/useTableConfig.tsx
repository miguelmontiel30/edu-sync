...
export function useTableConfig({ handleEdit, handleDelete }: TableConfigProps): TableConfigs {
        ...
        () => [
            {
                ...
                render: (subject: Subject) => (
                    <span className="font-outfit text-sm font-medium text-gray-800 dark:text-white/90">
                        {subject.name}
                    </span>
                ),
            },
            {
                ...
                sortable: true,
                render: (subject: Subject) => (
                    <span className="font-outfit text-sm text-gray-700 dark:text-white/70">
                        {subject.description}
                    </span>
                ),
            },
                    ...
                    return (
                        <span className="font-outfit text-sm text-gray-700 dark:text-white/70">
                            <Badge
                                ...
                                variant="light"
                                color={
                                    subject?.status?.status_id === SUBJECT_STATUS.SUBJECT_ACTIVE
                                        ? 'success'
                                        : subject?.status?.status_id ===
                                            SUBJECT_STATUS.SUBJECT_INACTIVE
                                          ? 'dark'
                                          : 'primary'
                                }
                            >
    ...
    const subjectListConfig: ItemsListConfig<Subject> = useMemo(
        () => ({
            ...
            description:
                'Aquí podrás ver todas las materias registradas, su información y gestionarlas. Puedes crear nuevas materias, editar las existentes o eliminarlas según sea necesario.',
    ...
    const deleteConfirmModalConfig = useMemo(
        () => ({
            ...
            recoveryInfo:
                'Esta acción puede ser revertida más adelante desde la sección de materias eliminadas.',
            warningTitle: '¡Atención!',
            warningMessage:
                'Estás a punto de eliminar una materia. Esta acción podría afectar a los grupos y evaluaciones asociados. Asegúrate de que no exista información importante vinculada a esta materia antes de continuar.',
            errorTitle: '¡Error!',
            errorMessage:
                'Ocurrió un error al intentar eliminar la materia. Por favor, intenta nuevamente.',
        }),
        [],
    );
